...
local function NewSpell(player, modifies, level)
    ...
    local nProjectiles = 5 + (modifiers.shotgun and modifers.shotgun * 3 or 0) + (level and (level-1)*2 or 0)
    local sprayAngle = 0.1 + (modifiers.fireball and modifiers.fireball * 0.05 or 0)
    local myDamage = 60 * (nProjectiles+baseN)/(nProjectiles*2) * (level and 1 + 0.25 * (level-1) or 1)
